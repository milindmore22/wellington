...
class Wellington_Magazine_Posts_Columns_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			sprintf( esc_html__( 'Magazine Posts: 2 Columns (%s)', 'wellington' ), wp_get_theme()->Name ), // Name.
			array(
				'classname' => 'wellington-magazine-posts-columns',
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_wellington_magazine_posts_columns', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_wellington_magazine_posts_columns', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	...
	function category_title( $args, $settings, $category_id, $category_title ) {
		...
		if ( ! empty( $widget_title ) ) :
			...
			if ( $category_id > 0 ) :
				...
				$link_title = sprintf( esc_html__( 'View all posts from category %s', 'wellington' ), get_cat_name( $category_id ) );
...
<?php
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'widget_wellington_magazine_posts_columns', 'widget' );
